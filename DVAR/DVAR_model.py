...
class AdaptationModule(tf.keras.layers.Layer):
    def __init__(self, embeddings, item_cate):
        ...
        # Convert embeddings tensor to numpy array if needed for copying & np.mean
        embeddings_np = embeddings.numpy() if hasattr(embeddings, 'numpy') else embeddings
        
        self.embeddings = embeddings_np
        self.cate_embedding = embeddings_np.copy()
    ...
    def call(self, inputs, **kwargs):
        user, item = inputs
        user = tf.cast(user, tf.int32)
        item = tf.cast(item, tf.int32)
...
class DVARModel(tf.keras.Model, ABC):
    def __init__(self, embeddings, item_cate, meta_paths, **kwargs):
        super(DVARModel, self).__init__(**kwargs)
    ...
    def custom_train(self, data):
        x, y = data
        user = tf.cast(x[:, 0], tf.int32)
        item = tf.cast(x[:, 1], tf.int32)
        path = tf.cast(x[:, 2], tf.int32)
    ...
    def get_config(self):
        return {
            ...
            "item_cate": self.AdaptationModel.item_cate.tolist(),  # assuming you saved it in AdaptationModule
            "meta_paths": [mp.numpy().tolist() for mp in [self.UICI, self.UUUI, self.UIUI, self.UIII, self.UII, self.UUI]]
        }
    ...
    def from_config(cls, config):
        embeddings = tf.convert_to_tensor(config["embeddings"], dtype=tf.float32)
        item_cate = np.array(config["item_cate"], dtype=np.int32)
        meta_paths = [tf.convert_to_tensor(mp, dtype=tf.int32) for mp in config["meta_paths"]]
        return cls(embeddings, item_cate, meta_paths)
