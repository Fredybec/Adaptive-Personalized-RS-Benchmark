...
class CoDBand:
	...
	def decide(self, pool_articles, userID):
		...
		for x in pool_articles:
			...
			scored_articles.append((x, x_pta)) ##
			if maxPTA < x_pta:
				articlePicked = x
				maxPTA = x_pta

		    # Sort articles by score (descending) and store internally
		scored_articles.sort(key=lambda item: item[1], reverse=True) ##
		self.last_ranked_articles = [x[0] for x in scored_articles] ##
		self.last_scores = [x[1] for x in scored_articles] ##
