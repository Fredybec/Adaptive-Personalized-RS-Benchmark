...
class experimentOneRealData(object):
    def __init__(self, namelabel, dataset, context_dimension, batchSize = 1 ,plot = True, Write_to_File = False):
        ...
        if self.dataset == 'LastFM':
            ...
            self.event_fileName = self.address + "/randUserOrderedTime_N5918_ObsMoreThan10.dat"
            relevant_items_per_user = {}
            print('---------Getting relevant items per user for MovieLens-------')
            with open('data/ml-1m/ratings.dat', 'r') as f:
                for line in f:
                    ...
                    if len(parts) != 4:
                        continue
                    uid, iid, rating, _ = parts
                    if float(rating) >= 4.0:
                        uid = int(uid)
                        iid = int(iid)
                        if uid not in relevant_items_per_user:
            ...
            FeatureVectorsFileName = Amazon_FeatureVectorsFileName
            self.event_fileName = self.address + "/randUserOrderedTime_N4068_ObsMoreThan3ForAmazon.dat"
            import json
            print('---------Getting relevant items per user for Amazon-------')
            relevant_items_per_user = {}
            with open('data/amazon/amazon.jsonl', 'r') as f:
                for line in f:
                    ...
                    if rating >= 4:
                        if uid not in relevant_items_per_user:
                            relevant_items_per_user[uid] = set()
                        relevant_items_per_user[uid].add(aid)
            print('---Algorithm started-----')
    ...
    def runAlgorithms(self, algorithms, startTime):
        ...
        with open(self.event_fileName, 'r') as f:
            ...
            for _, line in enumerate(f, 1):
                ...
                for alg_name, alg in algorithms.items():
                    ...
                    if alg_name not in AlgMetrics:
                        AlgMetrics[alg_name] = {k: {'precision': [], 'recall': [], 'f1': [], 'ndcg': []} for k in k_values}

                    relevant_items = relevant_items_per_user.get(userID, set())

                for k in k_values:
                    top_k_articles = alg.last_ranked_articles[:k]
                    recommended_ids = [a.article_id for a in top_k_articles]

                    tp = len(set(recommended_ids) & relevant_items)
                    precision = tp / k
                    ...
                    true_relevance = [1 if aid in relevant_items else 0 for aid in recommended_ids]
                    ideal_relevance = sorted(true_relevance, reverse=True)
                    ndcg = dcg(true_relevance) / (dcg(ideal_relevance) + 1e-9)

                    AlgMetrics[alg_name][k]['precision'].append(precision)
                    AlgMetrics[alg_name][k]['recall'].append(recall)
                    AlgMetrics[alg_name][k]['f1'].append(f1)
                    AlgMetrics[alg_name][k]['ndcg'].append(ndcg)
        ...
        for alg_name in AlgMetrics:
            print(f"\n===== {alg_name} Evaluation Metrics =====")
            for k in k_values:
                p = np.mean(AlgMetrics[alg_name][k]['precision'])
                r = np.mean(AlgMetrics[alg_name][k]['recall'])
                f1 = np.mean(AlgMetrics[alg_name][k]['f1'])
                ndcg = np.mean(AlgMetrics[alg_name][k]['ndcg'])
                print(f"@{k}  Precision: {p:.8f}, Recall: {r:.8f}, F1: {f1:.8f}, NDCG: {ndcg:.8f}")
...
if __name__ == '__main__':
    ...
    elif args.alg.lower() == 'codband':
        algorithms['CoDBand'] = CoDBand(
            v=config["CoDBand_v"],
            d=config["context_dimension"],
            alpha=config["CoDBand_alpha"],
            lambda_=config["lambda_"],
            NoiseScale=config["CoDBand_NoiseScale"],
            alpha_prior=config["alpha_prior"],
            tau_cd=config["tau"],
            memory_size=config["memory_size"],
            delta_1=config["delta_1"],
            delta_2=config["delta_2"],
            eta=config["CoDBand_eta"],
            disable_change_detector=config["disable_change_detector"],
            true_alpha_0=config["true_alpha_0"]
        )
    print(algorithms)
